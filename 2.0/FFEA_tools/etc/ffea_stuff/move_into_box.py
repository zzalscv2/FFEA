...
if len(sys.argv) != 2:
	sys.exit("Usage: python move_into_box.py [INPUT FFEA FILE]")
...
global_scale = float(blobs[0].scale)

#Calculate size and centroid of box
box_x = int(myparams.es_h) * 1.0/float(myparams.kappa) * int(myparams.es_N_x)/global_scale
box_y = int(myparams.es_h) * 1.0/float(myparams.kappa) * int(myparams.es_N_y)/global_scale
box_z = int(myparams.es_h) * 1.0/float(myparams.kappa) * int(myparams.es_N_z)/global_scale

box_centroid = vector3D(box_x/2.0, box_y/2.0, box_z/2.0)
...
#Translation vector
translate = vector3D(box_centroid.x - blobs_centroid.x, box_centroid.y - blobs_centroid.y, box_centroid.z - blobs_centroid.z)
print "Blob centroid: ", blobs_centroid.x, blobs_centroid.y, blobs_centroid.z
print "Box centroid: ", box_centroid.x, box_centroid.y, box_centroid.z
print "Translation: ", translate.x, translate.y, translate.z
#Output new node files

for i in range(int(myparams.num_blobs)):
	root_name = blobs[i].nodes[0:-5]
	print root_name
	outputnode = open(root_name + "_in_" + str(myparams.es_N_x) + str(myparams.es_N_x) + str(myparams.es_N_x) + "_box.node", "w")
	inputnode[i].seek(0)
	lines = inputnode[i].readlines()
	for j in range(5):
