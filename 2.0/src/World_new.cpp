...
int World::init(const char *FFEA_script_filename) {
	...
	// Parse for <param> block
	while(true) {
		if (get_next_script_tag(script_file, buf) == FFEA_ERROR) {
			FFEA_error_text();
			printf("Error when reading tag in %s\n", FFEA_script_filename);
			fclose(script_file);
			return FFEA_ERROR;
	    	}
	...
	// Check params for required params and unreasonable params
	...
	printf("Building system from <system> block...\n");
...
/* */
int World::read_and_build_system(FILE *script_file) {
	...
	// Save system beginning position
	system_beginning_pos = ;

	// Parse system
	printf("Entering <system> block...\n");
	for(i = 0; i < params.num_blobs; ++i) {

		// Set system_beginning_pos

		// Parse for <blob> block
		index = -1;
		while(true) {
			if (get_next_script_tag(script_file, buf) == FFEA_ERROR) {
				FFEA_error_text();
		...
		// Save blob beginning position
		blob_beginning_pos = ;

		for(j = 0; j < params.num_conformations[i]; ++j) {
			
			// Set blob_beginning_pos
			
			// Parse for <conformations> block
			index = -1;
			while(true) {
				if (get_next_script_tag(script_file, buf) == FFEA_ERROR) {
					FFEA_error_text();
			...
			while(true) {
				...
				if (strcmp(buf, "/system") == 0 || strcmp(buf, "/blob") == 0) {
					FFEA_error_text();
					printf("Error. Didn't find </conformation>, instead found %s when reading tag in %s\n", buf, FFEA_script_filename);
					fclose(script_file);
					return FFEA_ERROR;
				}
		...
		if(num_conformations[i] > 1) {
			...
			while(true) {
				...
				if (strcmp(buf, "kinetics") == 0) {
					break;
				}

			}

			// Save kinetics beginning position
			kinetics_beginning_pos = ;

			// Parse for <maps> block
			while(true) {
				if (get_next_script_tag(script_file, buf) == FFEA_ERROR) {
					FFEA_error_text();
					printf("Error when reading tag in %s\n", FFEA_script_filename);
			...
			while(true) {
				...
				if (feof(script_file)) {
		            		FFEA_error_text();
	         	   		printf("\tReached end of file before end of <maps> block\n");
	        	    		fclose(script_file);
	            			return FFEA_ERROR;
	        		}

				if (strcmp(buf, "/system") == 0 || strcmp(buf, "/blob") == 0 || strcmp(buf, "/kinetics") == 0) {
					FFEA_error_text();
					printf("Error. Didn't find </maps>, instead found %s when reading tag in %s\n", buf, FFEA_script_filename);
					fclose(script_file);
					return FFEA_ERROR;
				}
				...
				// Find which map and assign

			}

			// Set kinetics_beginning_pos
			...
			while(true) {
				...
				if (strcmp(buf, "/system") == 0 || strcmp(buf, "/blob") == 0) {
					FFEA_error_text();
					printf("Error. Didn't find </kinetics>, instead found %s when reading tag in %s\n", buf, FFEA_script_filename);
					fclose(script_file);
					return FFEA_ERROR;
				}
		...
		while(true) {
			...
			if (feof(script_file)) {
		           	FFEA_error_text();
	         	  	printf("\tReached end of file before end of <blobs> block\n");
	        	   	fclose(script_file);
	            		return FFEA_ERROR;
	        	}
			...
			if (strcmp(buf, "/system") == 0) {
				FFEA_error_text();
				printf("Error. Didn't find </blob>, instead found %s when reading tag in %s\n", buf, FFEA_script_filename);
				fclose(script_file);
				return FFEA_ERROR;
			}
...
int World::load_springs(char *fname) {
	...
	// Final line
	fscanf(in, "blob0 conformation0 node0 blob1 conformation1 node1 k l:\n");
    for (i = 0; i < num_springs; ++i) {
        if (fscanf(in, "%d %d %d %d %d %d %lf %lf\n", &spring_array[i].blob_index[0], &spring_array[i].conformation_index[0], &spring_array[i].node_index[0], &spring_array[i].blob_index[1], &spring_array[i].conformation_index[1], &spring_array[i].node_index[1], &spring_array[i].k, &spring_array[i].l) != 8) {
            FFEA_error_text();
	    fprintf(stderr, "%d %d %d %d %d %d %lf %lf\n", spring_array[i].blob_index[0], spring_array[i].conformation_index[0], spring_array[i].node_index[0], spring_array[i].blob_index[1], spring_array[i].conformation_index[1], spring_array[i].node_index[1], spring_array[i].k, spring_array[i].l);
