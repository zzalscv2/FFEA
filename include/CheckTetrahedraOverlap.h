...
class checkVars {
	...
	public:
		...
		scalar e_v1[6][3],e_v2[6][3];            // vectors edge-oriented
		int masks[4];			        // for each face of the first tetrahedron
								// stores the halfspace each vertex of the
								// second tetrahedron belongs to

		scalar P_V1[4][3], P_V2[4][3];           // differences between the vertices of the second (first) 
								//  tetrahedron
								// and the vertex 0  of the first(second) tetrahedron

		scalar  Coord_1[4][4],Coord_2[4][4];     // vertices coordinates in the affine space
