					...
					while(l_j != NULL) {
						if(l_i->index != l_j->index) {
							...
							if(f_i->daddy_blob != f_j->daddy_blob) {
								...
								f_i->set_vdw_bb_interaction_flag(true, f_j->daddy_blob->index);
								f_j->set_vdw_bb_interaction_flag(true, f_i->daddy_blob->index);
...
//			}
			...
			energy *= ApAq;
			f1->add_bb_vdw_energy_to_record(energy, f2->daddy_blob->index);
			f2->add_bb_vdw_energy_to_record(energy, f1->daddy_blob->index);
			...
			for(int j = 0; j < 3; j++) {
				...
				f1->add_bb_vdw_force_to_record(&force1, f2->daddy_blob->index);
//				printf("1:: %d %e %e %e\n", j, force1.x, force1.y, force1.z);
				...
				f2->add_bb_vdw_force_to_record(&force1, f1->daddy_blob->index);
