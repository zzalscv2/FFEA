...
//    f2->add_bb_vdw_energy_to_record(vol, f1->daddy_blob->blob_index);
    ...
    /**Calculates StericX forces modified with periodic boundary correction in distance calculation*/
void Steric_solverX::do_interaction(Face *f1, Face *f2, scalar * blob_corr){
 ...
 /*   #pragma omp critical
  ...
  //  exit(0);
    ...
    /* TRIAL 2 */
    arr3 force1, force2; //, n1_b;
    f1->vec3Vec3SubsToArr3Mod(f2, force1, blob_corr, f1_daddy_blob_index, f2_daddy_blob_index);
    arr3Normalise<scalar,arr3>(force1); // that is the direction of the force for f1 (backwards).
    ...
    */
    ...
    // scalar vol = f1->getTetraIntersectionVolume(f2);
    geoscalar vol, area;
    f1->getTetraIntersectionVolumeAndArea(f2,vol,area,blob_corr,f1_daddy_blob_index,f2_daddy_blob_index);
    //fprintf(stderr, "Vol = %f\nArea = %f\n", vol, area);
    //exit(0);
    area *= steric_factor;
    vol *= steric_factor;
