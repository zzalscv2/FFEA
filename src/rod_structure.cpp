...
Rod Rod::load_header(std::string filename){
    ...
    int n = 0;
    bool reading_current_rod = false; /** Make sure we're in the header info for our rod */
    ...
    for( std::string line; getline( infile, line ); ){
        ...
        if (n > 0 and line != rod_connections){
            ...
            /** Only read in the contents if we're in the right rod number */
            if (line_vec[0] == "HEADER" and std::stoi(line_vec[2]) == rod_no){
                reading_current_rod = true;
            }
            else if (line_vec[0] == "HEADER" and std::stoi(line_vec[2]) != rod_no){
                reading_current_rod = false;
            }
            
            /** Read in the contents */
            if (line_vec[0] == "version"){ this->rod_version = std::stod(line_vec[1]);}
            if (line_vec[0] == "length" and reading_current_rod){ this->length = std::stoi(line_vec[1]); length_set=true; }
            if (line_vec[0] == "num_elements" and reading_current_rod){ this->num_elements = std::stoi(line_vec[1]); }
            if (line_vec[0] == "num_rods" and reading_current_rod){ this->num_rods = std::stoi(line_vec[1]); }
        }   
    ...
    if (fabs(this->rod_version - rod_software_version) > 0.0000001){
        std::cout << "Serious warning: The rod input file is not the same version as this software. \n";
        std::cout << "Traj file version: " << this->rod_version << ", software version: " << rod_software_version << ".\n";
    }
...
Rod Rod::load_contents(std::string filename){
    ...
    for( std::string line; getline( infile, line ); ){
        std::vector<std::string> line_vec;
        boost::split(line_vec, line, boost::is_any_of(" "));
        if (line_vec[0] == "FRAME" and std::stoi(line_vec[3]) == rod_no)
