...
# perform coarsening			
def coarsen_surface(nodes, faces, new_nodes, new_faces, threshold):
	runs = 0
	while True:
		num_faces = len(faces)
		num_nodes = len(nodes)
...
if len(sys.argv) != 4:
	sys.exit("Usage: python netgen_surface_coarse_grainer [INPUT .SURF FILE] [OUTPUT .SURF FILE] [LOWER LENGTH THRESHOLD (ANGSTROMS)]")
...
coarsen_surface(nodes, faces, new_nodes, new_faces, length_thresh)
