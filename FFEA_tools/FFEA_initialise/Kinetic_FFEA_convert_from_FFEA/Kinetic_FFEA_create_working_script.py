...
for line in lines:
	if "<spring>" in line or "</spring>" in line or "spring_fname" in line:
		...
		if lvalue.strip() == "restart":
			...
			new_line = line.replace(rvalue, " 1e-06")
		elif lvalue.strip() == "pin":
			new_line = line.replace(rvalue, " working.pin")
			os.system("python /localhome/py09bh/Software/FFEA/FFEA_git/FFEA_tools/FFEA_initialise/Pin_tools/pin_none.py working.pin")
		else:
			new_line = line
	fout.write(new_line)
