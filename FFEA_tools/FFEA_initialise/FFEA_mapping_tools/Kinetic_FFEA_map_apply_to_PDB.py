...
if len(sys.argv) != 5:
	sys.exit("Usage: python " + os.path.basename(os.path.abspath(sys.argv[0])) + " [INPUT .pdb fname] [OUTPUT traj.out fname] [INPUT ffea .map fname] [FFEA scale factor]\n")
...
# Get nodes
input_atoms = FFEA_pdb.FFEA_pdb(innpdb, num_frames_to_read=10)
