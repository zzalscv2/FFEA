...
class FFEA_viewer_control_window:
	...
	def __init__(self, root, energy_thresh=1.0e6):
		...
		check_button_show_mesh = Checkbutton(display_flags_frame, text="Mesh (Linear)", variable=self.show_mesh, command=self.something_has_changed)
		check_button_show_mesh.pack(anchor=W)

		self.show_mesh_surf = IntVar()
		check_button_show_mesh_surf = Checkbutton(display_flags_frame, text="Mesh (Secondary)", variable=self.show_mesh_surf, command=self.something_has_changed)
		...
		check_button_show_material = Checkbutton(display_flags_frame, text="Material", variable=self.show_material, command=self.something_has_changed)
		...
		check_button_show_node_numbers = Checkbutton(display_flags_frame, text="Node Indices", variable=self.show_node_numbers, command=self.something_has_changed)
		check_button_show_node_numbers.pack(anchor=W)

		self.show_linear_nodes = IntVar()
		check_button_show_linear_nodes = Checkbutton(display_flags_frame, text="Node Indices (Linear)", variable=self.show_linear_nodes, command=self.something_has_changed)
		...
		self.choose_colour = Button(display_flags_frame, text="Blob Colour...", command=self.choose_blob_colour, bg=self.get_colour_code(self.blob_colour))
	...
	def load_ffea(self, ffea_fname):
		...
		if os.path.isfile(ffea_fname) == False:
			...
			self.cwd = os.path.dirname(ffea_fname)
	...
	def send_message_to_display(self, death):
		self.speak_to_display.send({	'animate':self.animate,
						...
						'display_flags':{	'selected_index': self.selected_index,
	...
	def get_updates_from_display(self):
		if self.display_window_exists == True:
			if self.speak_to_display.poll() == True:
				...
				if "death" in display_stuff.keys():
				...
				if "num_frames" in display_stuff.keys():
					self.frame_slider.config(to=display_stuff['num_frames'])

					if "current_frame" in display_stuff.keys():
				...
				if "pausing" in display_stuff.keys():
