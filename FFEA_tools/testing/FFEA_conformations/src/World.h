	...
	public:
		...
		*/
			...
			if(params.restart == 0) {
				...
				// First line in trajectory file should be an asterisk (used to delimit different steps for easy seek-search in restart code)
				fprintf(trajectory_out, "Number of Blobs %d\n\n*\n", params.num_blobs);
			...
			for(i = 0; i < params.num_blobs; i++) {
				...
				for(;;) {
					...
					if(strcmp(lvalue, "nodes") == 0) {
						...
						} else if(strcmp(rvalue, "nomasscg") == 0) {
							printf("\t\tSetting blob %d, linear solver = nomasscg (Preconditioned Jacobi Conjugate Gradient, no mass within system)\n", i);
							linear_solver = FFEA_NOMASS_CG_SOLVER;
						} else {
							FFEA_error_text();
							printf("\t\tThere is no solver option '%s'. Please use 'CG', 'direct' or 'nomasscg'.\n", rvalue);
							return FFEA_ERROR;
						}
						set_linear_solver = 1;
				...
				if(blob_array[i].init(node_filename, topology_filename, surface_filename, material_params_filename, stokes_filename, vdw_filename, pin_filename,
...
//		SurfaceElementLookup surface_element_lookup;
		...
		void print_trajectory_and_measurement_files(int step, double wtime)
		{
			...
			for(int i = 0; i < params.num_blobs; i++) {
				...
				// Write interblob measurements to file
				for(int j = i + 1; j < params.num_blobs; j++) {
					make_interblob_measurements(i, j);
				}
		...
		// Crazily inefficient and hacked in method of calculating these values. Please fix me post haste!
		void make_interblob_measurements(int blob_index_1, int blob_index_2)
		{
			scalar bb_area = 0.0, bb_force = 0.0, bb_energy = 0.0;
			vdw_solver.get_interblob_vdw_stuff(&blob_array[blob_index_1], &blob_array[blob_index_2], &bb_area, &bb_force, &bb_energy);
			fprintf(measurement_out[params.num_blobs], "%+e\t%+e\t%+e\t", bb_area, bb_force, bb_energy);
		}	
