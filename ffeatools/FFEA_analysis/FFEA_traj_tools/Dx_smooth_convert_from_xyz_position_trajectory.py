...
if len(sys.argv) != 6:
	sys.exit("Usage: python make_dx_map.py [INPUT 3 COLUMN X Y Z FILE] [NX] [NY] [NZ] [OUTPUT DX MAP FILENAME]")
infile_fname = sys.argv[1]
xn = int(sys.argv[2])
yn = int(sys.argv[3])
zn = int(sys.argv[4])
outfile_fname = sys.argv[5]
...
for i in range(len(pos_list)):
	...
	if atomx < maxx:
		histo[binx + 1][biny][binz] += 0.5
	num_atoms += 1

# normalise map
histo = (histo / num_atoms)
