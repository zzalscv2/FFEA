...
#							cmd.mplay()
	...
	def draw_springs(self, f):
		...
		cDrawable = []
		offset = 0
		cDrawable.extend([None for i in range(14 * len(self.springs.spring))])

		cc = [105.0/250.0, 105.0/250.0, 105.0/250.0]
		...
		for s in self.springs.spring:
			...
			cDrawable[offset + 14 * i] = CYLINDER
			cDrawable[offset + 14 * i + 1:offset + 14 * i + 4] = list(springjoints[0])
			cDrawable[offset + 14 * i + 4:offset + 14 * i + 7] = list(springjoints[1])
			cDrawable[offset + 14 * i + 7] = 1
			cDrawable[offset + 14 * i + 8:offset + 14 * i + 11] = cc
			cDrawable[offset + 14 * i + 11:offset + 14 * i + 14] = cc

			i += 1

		# Resize
		cDrawable = cDrawable[:14 * len(self.springs.spring) + offset]
		cmd.load_cgo(cDrawable, self.display_flags['system_name'] + "_Springs", f + 1)
