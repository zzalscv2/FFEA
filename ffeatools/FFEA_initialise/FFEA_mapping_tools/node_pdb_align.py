...
def print_progress_bar(start_text, num_iterations, num_done):
    ...
    blank_chars = "â–’"*int((bar_length*(1-ratio_done)))
    filled_chars = "â–ˆ"*int((bar_length*(ratio_done)))
    spacer = ""
    ...
    if len(str_to_write.decode('utf-8')) > cols:
        ...
        blank_chars = "â–’"*blank_char_num_new
        str_to_write = "\r"+textstr+filled_chars+blank_chars+"] "+str_done
...
if __name__ == "__main__" and hasattr(__builtin__, 'FFEA_API_mode') == False and sys.stdin.isatty():
    ...
    main(args.script, args.pdb, num_iterations=args.iterations, req_tolerance=args.tolerance, no_save=args.no_save, bindex=args.bindex, conf=args.cindex, node=args.node, traj=args.traj, num_candidates=args.candidates)            #!/usr/bin/env python2
# -*- coding: utf-8 -*-
"""
Created on Tue Sep 19 15:53:47 2017
