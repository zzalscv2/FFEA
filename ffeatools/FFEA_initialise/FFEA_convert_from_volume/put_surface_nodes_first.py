...
if len(sys.argv) != 5:
	sys.exit("Usage: python put_surface_nodes_first.py [NODE FILE] [TOPOLOGY FILE] [SURFACE FILE] [STOKES FILE]")
...
# check that this is indeed a stokes file
check = stokesfile.readline().rstrip()
print "file type = " + check
if check != 'walrus stokes radii file' and check != 'ffea stokes radii file':
	sys.exit("Error: walrus/ffea stokes radii file is expected for argument 4")

check = stokesfile.readline().rstrip().split()
num_stokes_nodes = int(check[1])
...
print "num nodes in stokes file = " + str(num_stokes_nodes)
...
for i in range(num_faces):
	...
	a[2] = int(a[2])
	a[3] = int(a[3])
	surface_node_list.append(a[1])
	surface_node_list.append(a[2])
	surface_node_list.append(a[3])
#	add_node(surface_node_list, a[1])
#	add_node(surface_node_list, a[2])
#	add_node(surface_node_list, a[3])

surface_node_list = set(surface_node_list)
...
non_surface_node_list = [nsn for nsn in non_surface_node_list if nsn not in surface_node_list]
...
surface_node_list = list(surface_node_list)
...
stokesfile = open(sys.argv[4], "w")
stokesfile.write('ffea stokes radii file\n')
stokesfile.write('num_nodes ' + str(num_stokes_nodes) + "\n");
for n in paired:
	stokesfile.write(n[2])
