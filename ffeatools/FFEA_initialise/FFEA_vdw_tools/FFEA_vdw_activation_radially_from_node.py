...
# Set up argparse
parser = _argparse.ArgumentParser(description="Activate the vdw faces based on predefined nodened nodes)")
...
parser.add_argument("-nind", action="store", help="Central Node Index")
parser.add_argument("-ind", action="store", help="VdW Face Index")

def activation_radially_from_node(vdw_fname, node_fname, surf_fname, output_fname, rad, nindex, index):

	# Check for problems
	if vdw_fname == None or node_fname == None or surf_fname == None or output_fname == None or rad == None or nindex == None:
		raise IOError
	...
	for i in range(surf.num_faces):
		...
		if np.linalg.norm(c - node.pos[nindex]) <= rad:
...
if sys.stdin.isatty() and hasattr(__builtin__, 'FFEA_API_mode') == False:
    ...
    try:
	activation_radially_from_node(args.i[0], args.n[0], args.s[0], args.o[0], args.r, args.nind, args.ind)
    except IOError:
	parser.print_help()
    except TypeError:
	parser.print_help()
