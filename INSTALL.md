...
This document gives instructions on how to build and install the FFEA package, consisting
of the FFEA runner and the FFEA tools. 
 
If you don't want to perform the cmake and make steps, then you can download the
program[here](https://bitbucket.org/FFEA/ffea/downloads/). In that case, make
sure that Python >=2.6 (but NOT Python 3!), NumPy, SciPy and Matplotlib are installed.
...
=============
   ...
   * [Python](https://www.python.org/) (>= 2.6), is used to run some tests to 
        Verify that the FFEA runner was correctly built, as well as in the FFEA tools.

   * [NumPy](http://www.numpy.org/), [SciPy](https://www.scipy.org/) and
        [Matplotlib](https://matplotlib.org/) are needed to run some tests as well as in several
        FFEAtools.
...
---------
   ...
   * [NETGEN](https://sourceforge.net/projects/netgen-mesher/)
     ...
     One of these is required for converting surface profiles into volumetric
     meshes, unless you already have a volumentric mesh. If you're not sure, we
     recommend TETGEN. Note: Not required for creating KOBRA rods. 

   * [PyMOL](https://pymol.org/) (>=1.8, though [1.8 is recommended](https://anaconda.org/mw/pymol))
     Highly recommened, needed to visualise systems and trajectories.
    
   * [Doxygen](http://www.doxygen.org) (>= 1.8) [OPTIONAL]   
     Used to build the documentation. Some mathematical formulae 
     will not render correctly if [LaTeX](https://www.tug.org/texlive/) is not
     found. If you are building the master branch, you can use the
     [online documentation](ffea.readthedocs.io) instead.
    
   * [MDAnalysis](https://www.mdanalysis.org/) (>=0.18.0)
     Used during rod parameterisation. If you don't plan to use KOBRA rods,
     you can ignore this.

The [Tutorial](\ref Tutorial) will explain how to use NETGEN, TETGEN and PyMOL.
...
It is generally advisable to configure and compile FFEA outside of the source tree. E.g.
...
it will bump ` Plugin FFEAplugin has been installed but initialization failed `.
All you need to do is  to restart PyMOL to use the plugin.
...
ffeatools provides a suite of command-line tools to initialise FFEA systems. FFEA also
provides a python API to accomplish these tasks. You can install it by entering the
ffea source folder and running 
...
extra packages that have proved to be useful for setting up the system to
simulate, as well as for analysing the results:
...
In the case they are not  installed in a standard place, you can help CMake either
through: 
...
Note: Automatically resolved dependencies
-----------------------------------------

FFEA uses Boost and Eigen. To make your life easier, **FFEA will automatically download and compile 
a subset of Boost (v. 1.63), and Eigen (v 3.3.4)** at configure time. Still, you are welcome to
use your own versions of the libraries.
